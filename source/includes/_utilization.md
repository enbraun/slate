...
**data**<br>`optional` | This parameter allows querying for planned and actual utilization. Possible values for this parameter can be any combination of 'planned,' 'actual,' 'requested,' and 'capacity' ('requested' when the view is set to project and 'capacity' when the view is set to resource). <br>_Default value of `data` is_ <span class="required">**`planned`**</span>.
