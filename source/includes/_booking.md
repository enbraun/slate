...
<span style="color:#b93d6a">`Booking`</span> object represents an assignment or schedule of <a href ="#resource" style="text-decoration:none">resource</a> on a certain <a href="#project" style ="text-decoration:none">project</a> or <a href="#tasks" style="text-decoration:none">task</a> for a particular time range.<a href ="#resource" style="text-decoration:none"> Resource </a>can be booked / scheduled  on a <a href="#project" style ="text-decoration:none">project </a>(and task) with a defined effort and capacity along with other custom defined fields.
...
**task** <br><span class="optional">`object`</span> | Task object defines what need to be done. Tasks could be one of the defined tasks of booking's object. 
...
**unit**  <br><span class="optional">`optional`</span>  | Unit in which effort is being defined. Unit value could be one of following. <li>1 for 'Percent of capacity' </li> <li>2 for 'Total Booking Hours' </li> <li>3 for 'Hours Per day' </li> <li> 4 for 'Full Time Equivalent'</li> <li> 5 for 'Time Per Day' </li>|
...
| **400** <br> <span class = "error">`Bad Request`</span> | Bad Request error occurs when a request is not malformed, syntactically incorrect, missing required parameters are  or any unknown parameter is passed. <br> Additionally, Bad request may also occur in one of these conditions :<ul><li>Invalid image file is provided.</li><li>booking's start_time is after its end_time.</li></ul> |
...
```shell
 curl -v -X GET "https://app.eresourcescheduler.cloud/rest/v1/bookings"\
  -H "Authorization: Bearer B8x5Vj1O65r6wnoV"
```
...
```shell 
curl -v -X GET \
"https://app.eresourcescheduler.cloud/rest/v1/bookings?offset=1&limit=1" \
 -H "Authorization: Bearer B8x5Vj1O65r6wnoV"
```
...
<span class="optional"><b>Query Param</b></span>
