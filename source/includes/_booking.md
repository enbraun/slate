...
**409** <br> <span class = "error">`Conflict`</span> | Conflict indicates that you are updating a recurring booking, so you must pass the parameter `update_connected_bookings` to replicate the changes to the related bookings. The value for the parameter `update_connected_bookings` can be **1**(for updating all bookings), **2**(for updating all future bookings) or **4**(for updating only this booking) as shown in the example requests.
